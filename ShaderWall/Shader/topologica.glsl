...
float noise2dT(vec2 uv)
{
    ...
    vec2 fl = floor(uv);
    uv = smooth + fl;
    return texture2D(iChannel0, (uv + 0.5)/iChannelResolution[0].xy).y;
    // use constant here instead?
}
float noise(vec3 uv)
{
...
float Density(vec3 p)
{
    ...
    // + sin(iGlobalTime)*0.5-1.95 + warp.x*4.0;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    float my=-iMouse.y/iResolution.y*10.0 + sin(iGlobalTime * 0.03)*0.2+0.2;
    //*PI/2.01;
    ...
    for (int i = 0; i < 37; i++)	// This is the count of how many times the ray actually marches.
    {
        ...
        inc = 1.9 + temp*0.05;
        // add temp because this makes it look extra crazy!
        density += temp * inc;
        t += inc;
    }
